...
export const ImmerseSection = () => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // Вычисляем положение секции в окне браузера
      if (rect.bottom > 0 && rect.top < windowHeight) {
        // Прогресс скролла: 0, когда верх секции только касается низа экрана,
        // 1, когда центр секции по центру экрана
        const scrollProgress = (windowHeight - rect.top) / (windowHeight + rect.height * 0.5);
        const clampedProgress = Math.max(0, Math.min(1, scrollProgress));

        // Смещение от +120px (спрятано глубоко за скалами) до 0px (поднято в небо)
    ...
    handleScroll(); // Первоначальный расчет при загрузке
  ...
  return (
    ...
    >
      {/* 1. Задний слой (z-0): Фоновое небо и верхний каньон (3840x2160, 16:9) */}
      <img
        src={immerseBg}
      ...
      {/* 2. Текст (z-10): Гигантская надпись IMMERSE с интерактивным параллакс-подъемом при скролле */}
      <div
        style={{
      ...
      {/* 3. Передний слой (z-20): Точный слой каньона (3840x1214, ровно 56.2% от высоты 2160), перекрывающий низ букв */}
      <img
      ...
      <div className="relative z-30 flex flex-col items-center justify-center text-center px-4 pb-6 sm:pb-8 md:pb-12 lg:pb-14 w-full max-w-3xl">
        ...
        <div className="bg-[#0b0806]/95 border-b-[3.5px] sm:border-b-4 border-b-[#e57d38] px-6 sm:px-12 md:px-16 lg:px-20 py-3.5 sm:py-5 md:py-6 shadow-[0_15px_40px_rgba(0,0,0,0.9)] max-w-full">
          <p className="text-xs sm:text-base md:text-lg lg:text-xl font-bold text-white tracking-[0.14em] sm:tracking-[0.18em] uppercase leading-snug sm:leading-relaxed">
            YOURSELF IN THE ATMOSPHERE<br />OF THE POST-APOCALYPSE
          </p>
        </div>
        ...
        <div className="relative mt-4 sm:mt-5 md:mt-6 group">
          {/* Деликатное оранжевое радиальное свечение */}
